...
@push('modal')
    ...
    @foreach ($materi as $materi)
        <div class="modal fade" id="lihat{{ $materi->id }}" tabindex="-1" aria-labelledby="data_peserta" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form action="{{ route('tutor.kelasku.data-peserta.update', [$kelas_id, $materi->id]) }}" method="POST">
                        @csrf
                        @method('put')
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            <div class="table-responsive">
                                <table class="table">
                                    <tbody>
                                        ...
                                        @if ($materi->created_at == $materi->update_at)
                                            <tr>
                                                <td>Tanggal Diperbarui</td>
                                                <td>:</td>
                                                <td>{{ $materi->update_at }}</td>
                                            </tr>
                                        @endif
                                        <tr>
                                            <td>Nama Materi</td>
                            ...
                            <div>
                                @foreach ($materi->uploadMateri as $fileMateri)
                                    <a href="{{ route('tutor.materi.download', [$kelas->id, $fileMateri->id]) }}"><i class="far fa-save"></i> {{ $fileMateri->materi }}</a><br>
                                @endforeach
                            </div>
                        </div>
                        <div class="modal-footer">
        ...
        function confirmDelete(e) {  
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type:'DELETE',
                        url:'{{route("tutor.kelasku.materi.destroy", [1,2])}}',
                        data:{
                            "_token": "{{ csrf_token() }}",
                        },
